...
push_app() {
    ...
    cf push ${CF_APP} --docker-image awjh/letters-of-credit -i 1 -m 384M --no-start --no-manifest
    date
...
start_app() {
    ...
    cf set-env ${CF_APP} REACT_APP_REST_SERVER_CONFIG "{\"webSocketURL\": \"wss://${REST_SERVER_URL}\", \"httpURL\": \"https://${REST_SERVER_URL}/api\"}"
